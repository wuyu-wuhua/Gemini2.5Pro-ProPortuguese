...
function getSafeTranslations() {
    ...
    return {
        ...
        pt: { 'untitled-chat': 'Без названия чата', 'history-title': 'История чата', 'no-history': 'Еще нет истории', 'welcome-message': 'Привет! Чем могу помочь?' }
    };
}
...
function switchLanguage(lang) {
    if (lang && lang === 'pt') { // Only pt is supported now
...
function applyTranslations(languageToApply) {
    const currentLang = languageToApply || localStorage.getItem('language') || 'pt';
    const trans = getSafeTranslations();
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    langOptions.forEach(option => {
        ...
        // Since only pt is supported, ensure only pt option can be active if it exists
        if (option.getAttribute('data-lang') !== 'pt') option.style.display = 'none'; else option.style.display = 'block';
    });
...
if (langOptions) {
    langOptions.forEach(option => {
        ...
        newOption.addEventListener('click', (e) => {
            ...
            if (lang === 'pt') { // Only allow switching to pt
                switchLanguage(lang);
            }
        });
    });
